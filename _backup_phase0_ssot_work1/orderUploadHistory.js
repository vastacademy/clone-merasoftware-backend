...
const getOrderUploadHistory = async (orderId, { userId } = {}) => {
  ...
  return requests.map((request) => {
    const files = (request.files || []).map((file) => {
      // Files are removed from Drive once they expire (fileCleanupScheduler.js), so an
      // expired row must not be offered for download — the link would simply fail.
      const isExpired = Boolean(file.expirationDate) && new Date(file.expirationDate) < now;
      return {
        id: String(file._id || ""),
    ...
    return {
      ...
      files,
      // A request whose files have all expired can still be listed, but nothing in it
      // is downloadable — the ZIP route uses the same test.
      hasDownloadableFiles: files.some((file) => !file.isExpired && file.driveFileId),
    };
  });
};
