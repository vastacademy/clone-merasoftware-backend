...
const retireOrDeletePlan = async (req, res) => {
  try {
    ...
    // Remember what isHidden was so restoring puts the plan back exactly as it was.
    // Without this, a plan that was live before retiring would come back hidden and
    // the admin would have to re-enable it by hand — losing state, not protecting it.
    plan.hiddenBeforeRetire = Boolean(plan.isHidden);
    // Retiring implies it is off sale; keeping isHidden in step means every
    // pre-existing catalog filter that already checks isHidden also excludes it.
    plan.isHidden = true;
    await plan.save();
